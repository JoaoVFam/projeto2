...
function buscaCategoriaProduto($conexao, $categoria_id) {
	$query = "select * from categorias where id = {$categoria_id}";
	$resultado = mysqli_query($conexao, $query);
	return mysqli_fetch_assoc($resultado);
}
function adicionaProduto($conexao, $nome, $preco, $descricao, $categoria_id, $nome_imagem) {
...
function buscaUltimosProdutos($conexao) {
	$produtos = array();
