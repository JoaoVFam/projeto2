...
if(!empty($_FILES['imagem_produto']['name']) && preg_match('/^image\/(pjpeg|jpeg|png|gif|bmp|jpg)$/', $_FILES['imagem_produto']['type'])) {  //Teste se uma imagem válida foi enviada
	$imagem_produto = $_FILES['imagem_produto']['name'];
	$destino = 'anexo/' . $_FILES['imagem_produto']['name']; 
	$imagem_tmp = $_FILES['imagem_produto']['tmp_name'];
	if(insereProduto($conexao, $nome, $preco, $descricao, $categoria_id, $imagem_produto)) { //Teste se foi possível adicionar o produto sem erro
