...
if(!empty($_FILES['nome_imagem']['name']) && preg_match('/^image\/(pjpeg|jpeg|png|gif|bmp|jpg)$/', $_FILES['nome_imagem']['type'])) {
	...
	$destino = 'img/' . $_FILES['nome_imagem']['name']; 
	...
	if(adicionaProduto($conexao, $nome, $preco, $descricao, $categoria_id, $nome_imagem)) {
		...
		<div class="alert alert-success" role="alert">
			<p>Produto <?=$nome?> adicionado com sucesso!</p>
		</div>
	<?php } else {
	...
	<?php }
} elseif (adicionaProduto($conexao, $nome, $preco, $descricao, $categoria_id, 'default.png')) {?>
	<div class="alert alert-success" role="alert">
		<p>Produto <?=$nome?> adicionado com sucesso!</p>
	</div>
<?php } else{
